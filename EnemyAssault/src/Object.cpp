...
void Object::Render(GLint pointsX[], GLint pointsY[])
{
	glColor3f(0.0f, 1.0f, 0.0f);

	glBegin(GL_QUADS);

	for(int i = 0; i < sizeof(pointsX); i++)
	{
		for(int j = 0; j < sizeof(pointsY); j++)
		{
			pointsArrayX[i] = pointsX[i];
			pointsArrayY[j] = pointsY[j];
		}
		glVertex2i(pointsArrayX[i], pointsArrayY[i]);
	}
